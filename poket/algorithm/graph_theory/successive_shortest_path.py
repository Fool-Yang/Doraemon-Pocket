...
def SuccessiveShortestPath(G, C, s, t):
    ...
    while augmenting_path_found:
        ...
        if augmenting_path_found:
            ...
            more_flow = True
            while more_flow:
                more_flow = _send_flow(Gr, Gs, {s}, inf, s, t)
                total_flow += more_flow
                total_cost += more_flow*Potential[t]
    return Gr, total_flow, total_cost

def _send_flow(Gr, Gs, Upstream_Nodes, input_flow, u, t):
    if u == t:
        return input_flow
    ...
    for v in Gs[u]:
        room = Gs[u][v]
        # the shortest-path subgraph creates a DAG
        if v not in Upstream_Nodes and room > 0:
            output_flow = input_flow
            # find bottleneck
            if room < output_flow:
                output_flow = room
            output_flow = _send_flow(Gr, Gs, Upstream_Nodes, output_flow, v, t)
            if output_flow:
